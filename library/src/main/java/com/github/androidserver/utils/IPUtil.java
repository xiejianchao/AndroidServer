...
public class IPUtil {
    ...
    public static String getBroadcastIp(Context context) {
        ...
        if (wifiManager.isWifiEnabled()) {
            ...
            ip = split[0] + "." + split[1] + "." + split[2] + "." + (255 - Integer.parseInt(dhcpInfos.split("\\.")[3]));
        } else {
            String info = getNetInfo();
            String[] split = info.split(",");
            String ipStr = split[0];
            String netMask = split[1];
            String[] split1 = ipStr.split("\\.");
            ip = split1[0] + "." + split1[1] + "." + split1[2] + "." + (255 - Integer.parseInt(netMask.split("\\.")[3]));
        }
        return ip;
    }
